	...
	const apiDatasets: ApiDataset[] = [
		{
			...
			fetchFn: async () => {
				...
				const targetCountries = allCountries.filter((country: any) => 
					countries.some(target => 
						country.name.common.toLowerCase().includes(target.replace('unitedstates', 'united states'))
					)
				).slice(0, 5);
		...
		},
		{
			...
			fetchFn: async () => {
				...
				const stocks = [
					...
					{ symbol: 'MSFT', name: 'Microsoft', startPrice: 350, volatility: 0.018, color: '#00a1f1' },
	...
	async function fetchData(dataset: ApiDataset) {
		...
		try {
			...
			console.log('Data summary:', {
				...
				dateRange: data[0]?.data ? `${data[0].data[0]?.date} to ${data[0].data[data[0].data.length - 1]?.date}` : 'N/A'
			});
		} catch (err) {
...
<div class="api-demo-container">
	<div class="demo-header">
		<h1>Advanced Multi-Line Chart API Demo</h1>
		<p class="subtitle">
			Testing your high-performance line chart component with real-world APIs
		</p>
	</div>
	...
	{#if loading}
		<div class="loading-container">
			...
			<p>Loading {selectedDataset?.name}...</p>
		</div>
	{/if}
	...
	{#if error}
		<div class="error-container">
			...
			<button onclick={() => selectedDataset && fetchData(selectedDataset)}>
				🔄 Retry
			</button>
		</div>
	{/if}
	...
	<!-- API Information -->
	<div class="api-info">
		<h3>APIs Used in This Demo</h3>
		<div class="api-list">
			<div class="api-item">
				<strong>REST Countries API</strong> - Country population and statistics (no auth required)
				<br><small>https://restcountries.com - Reliable government data, no rate limits</small>
			</div>
			<div class="api-item">
				<strong>Nager.Date API</strong> - Public holidays data (no auth required)
				<br><small>https://date.nager.at - Official holiday calendars, very stable</small>
			</div>
			<div class="api-item">
				<strong>JSONPlaceholder</strong> - Fake REST API for testing (no auth required)
				<br><small>https://jsonplaceholder.typicode.com - Ultra-reliable test data</small>
			</div>
			<div class="api-item">
				<strong>Open-Meteo API</strong> - Free weather forecasts (no auth required)
				<br><small>https://open-meteo.com - No rate limits, no API key needed</small>
			</div>
			<div class="api-item">
				<strong>Synthetic Data</strong> - Generated datasets for performance testing
				<br><small>Mathematical functions and random walks</small>
			</div>
		</div>
	</div>
</div>
